...
def test_degree_features():
    edge_index = torch.tensor([[0, 0, 1, 1, 2, 2, 3, 4, 5, 6], [1, 2, 3, 4, 5, 6, 7, 7, 8, 8]], dtype=torch.long)
    ...
    assert torch.allclose(data.x, torch.cat([expected_out_degree, expected_in_degree], dim=-1))
